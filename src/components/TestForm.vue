        ...
        <div class="flex flex-col justify-center items-center flex-colitems-center">
          <div class="flex flex-col justify-center mb-2 items-center shadow-lg border-gray-300 border-2 p-2 w-4/5 bg-blue-50 rounded-3xl">
            ...
            <nav class="my-3" aria-label="question-navigation">
              <ul class="inline-flex items-center -space-x-px">
                ...
                <li v-for="(question, index) in this.activeTest.questions" :key='index'>
                  <button @click="this.$store.commit('setQuestion', index)" class="px-3 py-2 leading-tight text-gray-500 bg-white border border-gray-300 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                      {{ index + 1 }}
                      <!-- TODO: сделать проверку есть ли ответ на вопрос и менять bg-green-500 на bg-red-500 в зависимости от этого
                               а еще можно придумать обводку для текущего 
                      -->
                  </button>
                </li>
          ...
          <div class="justify-center shadow-lg border-gray-300 border-2 p-[20px] w-4/5 bg-blue-50 rounded-3xl">
              ...
              <div class="flex justify-evenly mt-5">
                ...
                <button disabled v-else-if="!testIsComplete" class="tooltip text-white bg-green-700 hover:bg-green-800 font-medium rounded-full text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700" type="button"
                        @click="this.$store.dispatch('addTestToResult')">
                  <p class="p-2 font-bold">Отправить результат</p>
                  <span class="tooltip__text">Вы еще не прошли вопросы:<br>1, 2, 3</span>
                </button>
                <button v-else-if="testIsComplete" class="tooltip text-white bg-green-700 hover:bg-green-800 font-medium rounded-full text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700" type="button"
                        @click="this.$store.dispatch('addTestToResult'); 
